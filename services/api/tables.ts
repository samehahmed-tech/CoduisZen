...
export const tablesApi = {
    ...
    split: (data: { sourceTableId: string; targetTableId: string; branchId: string; items: Array<{ id?: string; name: string; price: number; quantity: number }>; reference_id?: string }) =>
        apiRequest<any>('/tables/split', { method: 'POST', body: JSON.stringify(data) }),
    merge: (data: { sourceTableId: string; targetTableId: string; branchId: string; items: Array<{ id?: string; name: string; price: number; quantity: number }>; reference_id?: string }) =>
