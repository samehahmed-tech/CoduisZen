...
const INSTALL_DIR = installDirArg ? path.resolve(installDirArg.slice('--install-dir='.length).replace(/^"|"$/g, '')) : '';
const appRequire = INSTALL_DIR ? createRequire(path.join(INSTALL_DIR, 'package.json')) : require;
const XLSX = appRequire('xlsx');
const dotenv = appRequire('dotenv');
const mssql = appRequire('mssql/msnodesqlv8');
const catalog = require('./rosa-cafe-catalog-data.cjs');

// The package may be extracted inside the application folder or run from a
// standalone folder. Search both locations for the customer's environment.
const envRoots = [
  INSTALL_DIR,
  process.cwd(),
...
for (const envRoot of [...new Set(envRoots)]) {
...
function configuredDatabaseName(connectionString) {
  return connectionString.match(/(?:^|;)Database=([^;]+)/i)?.[1]?.trim() || 'CoduisZen';
}

function masterConnectionString(connectionString) {
  if (/(?:^|;)Database=[^;]*/i.test(connectionString)) {
    return connectionString.replace(/(?:^|;)Database=[^;]*/i, (match) => match.replace(/Database=[^;]*/i, 'Database=master'));
  }
  return `${connectionString};Database=master`;
}

async function connectToConfiguredDatabase(connectionString) {
  try {
    return await new mssql.ConnectionPool({ connectionString, requestTimeout: 600000 }).connect();
  } catch (firstError) {
    if (!/Cannot open database|login failed/i.test(String(firstError?.message || firstError))) throw firstError;

    // The installed services usually run as NT AUTHORITY\\SYSTEM. When the
    // customer launches this package as an Administrator, that interactive
    // Windows account may not yet have a user inside CoduisZen. Grant only
    // that account access, then retry the exact connection from .env.
    const databaseName = configuredDatabaseName(connectionString);
    const master = await new mssql.ConnectionPool({ connectionString: masterConnectionString(connectionString), requestTimeout: 600000 }).connect();
    try {
      const request = master.request();
      const identity = await request.input('databaseName', mssql.NVarChar(128), databaseName)
        .query('SELECT SUSER_SNAME() AS login_name, DB_ID(@databaseName) AS database_id');
      const loginName = String(identity.recordset?.[0]?.login_name || '').trim();
      const databaseId = identity.recordset?.[0]?.database_id;
      if (!loginName) throw firstError;
      if (!databaseId) throw new Error(`The installed SQL Server does not contain database "${databaseName}".`);
      const quotedLogin = `[${loginName.replace(/]/g, ']]')}]`;
      const quotedDatabase = `[${databaseName.replace(/]/g, ']]')}]`;
      await master.request().batch(`
        USE ${quotedDatabase};
        IF USER_ID(N'${loginName.replace(/'/g, "''")}') IS NULL
          CREATE USER ${quotedLogin} FOR LOGIN ${quotedLogin};
        IF IS_ROLEMEMBER(N'db_owner', N'${loginName.replace(/'/g, "''")}') = 0
          ALTER ROLE [db_owner] ADD MEMBER ${quotedLogin};
      `);
    } finally {
      await master.close();
    }
...
async function importData(tx, target, menu, inventory, recipes, report) {
  ...
  for (let i = 0; i < menu.categories.length; i++) {
    const sourceCategory = menu.categories[i]; categoryIds.set(sourceCategory.id, sourceCategory.id);
    const category = sourceCategory.nameAr === 'Extra'
      ? { ...sourceCategory, name: 'Extras', nameAr: 'إضافات' }
      : sourceCategory;
    await request(tx, `INSERT INTO menu_categories (id,name,name_ar,sort_order,is_active,menu_ids,target_order_types) VALUES (@id,@name,@nameAr,@sort,1,@menuIds,@types)`, {
      id: category.id, name: category.name, nameAr: category.nameAr, sort: i * 10, menuIds: '["menu-1"]', types: '["DINE_IN","TAKEAWAY","DELIVERY"]'
    });
  }
  const menuByArabic = new Map(); const menuByEnglish = new Map();
  const explicitChoiceDefinitions = new Map([
    [norm('أعشاب (قرفة - ينسون - جنزبيل - نعناع)'), { baseName: 'أعشاب', groupName: 'Flavor', groupNameAr: 'النكهة', options: ['قرفة', 'ينسون', 'جنزبيل', 'نعناع'] }],
    [norm('موكا ( دارك -وايت)'), { baseName: 'موكا', groupName: 'Chocolate', groupNameAr: 'نوع الشوكولاتة', options: ['دارك', 'وايت'] }],
    [norm('ايس موكا  (دارك - وايت)'), { baseName: 'ايس موكا', groupName: 'Chocolate', groupNameAr: 'نوع الشوكولاتة', options: ['دارك', 'وايت'] }],
    [norm('موكتيل ريدبل ( فراوله  -بلو بيري- جوز هند_باشن فروت)'), { baseName: 'موكتيل ريدبل', groupName: 'Flavor', groupNameAr: 'النكهة', options: ['فراولة', 'بلو بيري', 'جوز هند', 'باشن فروت'] }],
  ]);
  const choiceVariant = (name) => {
    const explicit = explicitChoiceDefinitions.get(norm(name));
    if (explicit) return explicit;
    const match = clean(name).match(/^(.*?)\s*\(([^()]*)\)\s*$/);
    if (!match || !/[\/_-]/.test(match[2])) return null;
    const options = match[2].split(/\s*[-\/_]\s*/).map(clean).filter(Boolean);
    if (options.length < 2) return null;
    return { baseName: clean(match[1]), groupName: 'Choice', groupNameAr: 'الاختيار', options };
  };
  const displayNameAr = (item) => item.category.nameAr === 'Extra'
    ? clean(item.nameAr).replace(/^اضافه\s*/i, 'إضافة ').replace(/\s*\([^)]*\)\s*$/, '').trim()
    : clean(item.nameAr);
  ...
  for (const item of menu.items) {
    ...
    const choice = item.category.nameAr === 'Extra' ? null : choiceVariant(item.nameAr);
    const baseName = variant?.baseName || choice?.baseName || displayNameAr(item);
    ...
    group.variants.push({ item, variant, choice });
  }
  ...
  report.importedModifierGroups = groupedItems.filter((entry) => entry.variants.some((variant) => variant.choice)).length;
  for (let i = 0; i < groupedItems.length; i++) {
    const item = groupedItems[i]; const itemId = id('rosa-item', `${item.category.name}-${item.nameAr}`);
    const englishName = item.variants.map((entry) => Object.entries(menuAliases).find(([, ar]) => norm(ar) === norm(entry.item.nameAr))?.[0]).find(Boolean) || item.name;
    ...
    const choiceEntry = item.variants.find((entry) => entry.choice)?.choice;
    const modifierGroups = choiceEntry ? [{
      id: `choice-${slug(itemId)}`,
      name: choiceEntry.groupName,
      nameAr: choiceEntry.groupNameAr,
      minSelection: 1,
      maxSelection: 1,
      options: choiceEntry.options.map((optionName) => ({
        id: `option-${slug(`${itemId}-${optionName}`)}`,
        name: optionName,
        nameAr: optionName,
        price: 0,
        isAvailable: true,
  ...
  for (const source of recipes) {
    ...
    const usableSourceIngredients = source.ingredients.filter((ing) => number(ing.quantity) > 0);
    report.removedZeroQuantityIngredients += source.ingredients.length - usableSourceIngredients.length;
    if (!usableSourceIngredients.length) {
      report.skippedEmptyRecipes.push({ recipe: source.name, source: source.sourceSheet, row: source.sourceRow });
      continue;
    }
    ...
    if (seenRecipeKeys.has(recipeKey)) {
      report.duplicateRecipeMappings.push({ item: item.nameAr, source: source.name, size: source.size, sourceSheet: source.sourceSheet, sourceRow: source.sourceRow });
      continue;
    }
    seenRecipeKeys.add(recipeKey);
    const recipeId = id('rosa-recipe', `${item.id}-${selectedSize?.id || source.size || 'default'}`);
    const resolvedIngredients = [];
    for (const ing of usableSourceIngredients) {
      const stock = resolveInv(ing.name);
      if (!stock) {
        report.unmatchedIngredients.push({ recipe: source.name, ingredient: ing.name, source: source.sourceSheet, row: source.sourceRow });
        continue;
      }
      resolvedIngredients.push({ ing, stock });
    }
    if (resolvedIngredients.length !== usableSourceIngredients.length) {
      throw new Error(`Recipe mapping is incomplete for "${source.name}" (${source.sourceSheet} row ${source.sourceRow}).`);
    }
    ...
    for (const { ing, stock } of resolvedIngredients) {
...
async function validateImportedRecipes(tx, report) {
  const result = await request(tx, `
    SELECT
      (SELECT COUNT(*) FROM dbo.recipes) AS recipe_count,
      (SELECT COUNT(*) FROM dbo.recipe_ingredients) AS ingredient_count,
      (SELECT COUNT(*) FROM dbo.recipes r WHERE NOT EXISTS (
        SELECT 1 FROM dbo.recipe_ingredients ri WHERE ri.recipe_id = r.id
      )) AS empty_recipe_count,
      (SELECT COUNT(*) FROM dbo.recipe_ingredients WHERE quantity <= 0) AS zero_quantity_ingredient_count,
      (SELECT COUNT(*) FROM dbo.menu_items mi WHERE NOT EXISTS (
        SELECT 1 FROM dbo.recipes r WHERE r.menu_item_id = mi.id
      )) AS menu_items_without_recipes
  `);
  const row = result.recordset?.[0] || {};
  report.validation = {
    importedRecipes: Number(row.recipe_count || 0),
    importedRecipeIngredients: Number(row.ingredient_count || 0),
    emptyRecipes: Number(row.empty_recipe_count || 0),
    zeroQuantityIngredients: Number(row.zero_quantity_ingredient_count || 0),
    menuItemsWithoutRecipes: Number(row.menu_items_without_recipes || 0),
  };
  if (report.validation.emptyRecipes > 0 || report.validation.zeroQuantityIngredients > 0) {
    throw new Error(`Recipe validation failed: empty recipes=${report.validation.emptyRecipes}, zero-quantity ingredients=${report.validation.zeroQuantityIngredients}.`);
  }
}
...
async function main() {
  ...
  const pool = await connectToConfiguredDatabase(process.env.DATABASE_URL);
  ...
  const report = { generatedAt: new Date().toISOString(), target, existing, prepared: { categories: menu.categories.length, menuItems: menu.items.length, inventoryItems: inventory.length, recipes: recipes.length }, unmatchedRecipes: [], unmatchedIngredients: [], duplicateRecipeMappings: [], removedZeroQuantityIngredients: 0, skippedEmptyRecipes: [] };
  ...
  if (!COMMIT) {
    ...
    try {
      ...
      await validateImportedRecipes(previewTx, report);
  ...
  try {
    ...
    await validateImportedRecipes(tx, report);
    await tx.commit();
    fs.writeFileSync(OUT_REPORT, JSON.stringify(report, null, 2), 'utf8');
