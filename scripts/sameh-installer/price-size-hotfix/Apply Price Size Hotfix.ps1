...
function Run-Task([string]$action, [string]$name) {
    try { & schtasks.exe "/$action" /TN $name 2>$null | Out-Null } catch { }
}
...
$payloadDataFix = Join-Path $PSScriptRoot 'payload\data-maintenance-fix'
foreach ($path in @($node, $backup, $payloadServer, $payloadFrontend, $payloadDataFix)) { if (-not (Test-Path -LiteralPath $path)) { throw "Missing FIX payload: $path" } }

$stamp = Get-Date -Format 'yyyyMMdd-HHmmss'
$rollback = Join-Path $InstallDir "updates\price-size-rollback-$stamp"
$currentServer = Join-Path $InstallDir 'dist-server\index.cjs'
$currentFrontend = Join-Path $InstallDir 'dist'
$currentDataFix = Join-Path $InstallDir 'data-maintenance-fix'
...
try {
    ...
    if (Test-Path $currentDataFix) { Copy-Item $currentDataFix (Join-Path $rollback 'data-maintenance-fix') -Recurse -Force }
    ...
    if (Test-Path $currentDataFix) { Remove-Item $currentDataFix -Recurse -Force }
    Copy-Item $payloadDataFix $currentDataFix -Recurse -Force
    ...
    for ($attempt = 0; $attempt -lt 45; $attempt++) {
        try { $health = Invoke-RestMethod 'http://127.0.0.1:3001/api/health' -TimeoutSec 3; if ($health.health.services.database.status -eq 'CONNECTED') { Write-Host 'SUCCESS: Price/size FIX 1.1.27 applied.' -ForegroundColor Green; exit 0 } } catch { }
        Start-Sleep -Seconds 2
    }
    ...
    if (Test-Path (Join-Path $rollback 'data-maintenance-fix')) { Remove-Item $currentDataFix -Recurse -Force -ErrorAction SilentlyContinue; Copy-Item (Join-Path $rollback 'data-maintenance-fix') $currentDataFix -Recurse -Force }
    Start-SystemTasks
