param([string]$OutputPath = '.\artifacts\discount-fix')
...
$stage = Join-Path $output 'RestoFlow Discount Codes Fix'
...
Copy-Item -Path (Join-Path $PSScriptRoot 'discount-fix\*') -Destination $stage -Recurse -Force
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'daily-order-number-fix\migrate-daily-order-number.cjs') -Destination $stage -Force
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'daily-order-number-fix\migrate-daily-order-number.sql') -Destination $stage -Force
...
$zip = Join-Path $output 'RestoFlow Discount Codes Fix.zip'
Compress-Archive -Path (Join-Path $stage '*') -DestinationPath $zip -CompressionLevel Optimal
$zipHash = (Get-FileHash -LiteralPath $zip -Algorithm SHA256).Hash.ToLowerInvariant()
[IO.File]::WriteAllText((Join-Path $output 'RestoFlow Discount Codes Fix.sha256.txt'), "$zipHash  RestoFlow Discount Codes Fix.zip`r`n", [Text.UTF8Encoding]::new($false))
