...
if (-not $isAdmin) { throw 'Run Apply Discount Fix.bat as Administrator.' }
...
$dailyNumberMigration = Join-Path $PSScriptRoot 'migrate-daily-order-number.cjs'
$dailyNumberSql = Join-Path $PSScriptRoot 'migrate-daily-order-number.sql'
foreach ($required in @($node, $backupScript, $currentServer, $currentFrontend, $payloadServer, $payloadFrontend, $payloadManifest, $dailyNumberMigration, $dailyNumberSql)) {
...
$rollback = Join-Path $InstallDir "updates\discount-fix-rollback-$timestamp"
$logDir = Join-Path $InstallDir 'logs'
New-Item -ItemType Directory -Path $rollback, $logDir -Force | Out-Null
$log = Join-Path $logDir "discount-fix-$timestamp.log"
...
try {
    ...
    $migrationResult = & $node $dailyNumberMigration "--install-dir=$InstallDir" 2>&1
    $migrationResult | Add-Content -LiteralPath $log -Encoding UTF8
    if ($LASTEXITCODE -ne 0) { throw "Daily order number migration failed: $migrationResult" }
    ...
    @{ version = '1.1.15-daily-order-number'; appliedAt = (Get-Date).ToString('o'); rollback = $rollback } |
        ConvertTo-Json | Set-Content -LiteralPath (Join-Path $InstallDir 'discount-fix-state.json') -Encoding UTF8
    "SUCCESS $(Get-Date -Format o)" | Add-Content -LiteralPath $log -Encoding UTF8
    Write-Host "`nSUCCESS. Discount Codes Fix applied. Log: $log" -ForegroundColor Green
    exit 0
} catch {
