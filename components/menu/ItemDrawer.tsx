...
export const ItemDrawer: React.FC<Props> = ({
  ...
  return (
    <>
      <div className="fixed inset-0 z-[100] bg-slate-900/60 backdrop-blur-md animate-fade-in" onClick={onClose} />
      
      <div className={`fixed inset-y-6 md:inset-y-8 left-1/2 -translate-x-1/2 z-[130] flex flex-col bg-card shadow-2xl max-w-[1100px] w-[calc(100vw-2rem)] xl:w-[calc(100vw-6rem)] transition-all duration-500 animate-slide-up overflow-hidden border border-border/40 rounded-[2rem]`} dir={lang === "ar" ? "rtl" : "ltr"}>
        
        {/* Top Floating Header */}
        <div className="shrink-0 flex items-center justify-between px-8 py-5 border-b border-border/30 bg-elevated/40 relative z-20">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-primary/20 to-primary/5 border border-primary/20 flex items-center justify-center text-primary shadow-sm">
                <ShoppingBag size={24} />
            </div>
            <div>
        ...
        <div className="flex-1 flex overflow-hidden bg-card/50">
          ...
          <div className="w-64 shrink-0 border-r border-border/30 bg-elevated/20 flex flex-col rtl:border-l rtl:border-r-0 overflow-y-auto pos-scroll">
             <div className="p-6">
                <h3 className="text-[10px] font-black text-muted uppercase tracking-[0.2em] mb-4">
                  {lang === 'ar' ? 'مراحل الإعداد' : 'Configuration Steps'}
                </h3>
                <div className="space-y-1.5 p-1 relative z-10">
                   {tabs.map((t, i) => (
                      <button
                        key={t.id}
                        onClick={() => setTab(t.id)}
                      ...
                      >
                         ...
                         <span className="text-[13px] tracking-wide mt-0.5">{lang === "ar" ? t.ar : t.en}</span>
                         {tab === t.id && (
             ...
             <div className="mt-auto p-6 bg-gradient-to-t from-card/80 to-transparent">
          ...
          {/* Main Content Area */}
          <div className="flex-1 overflow-y-auto p-8 pos-scroll relative">
             <div className="max-w-4xl mx-auto pb-20">
               ...
               {tab === "BASIC" && (
                 <div className="space-y-8 animate-fade-in relative z-10">
                    <div className="grid lg:grid-cols-[1fr_minmax(0,340px)] gap-6">
                       <div className="glass-2 p-6 flex flex-col gap-5 border-border/40">
                       ...
                       <div className="glass-2 p-6 flex flex-col gap-6 border-border/40">
                         <h3 className="text-sm font-black text-main uppercase tracking-widest flex items-center gap-2">
                          ...
                          <div className="flex-1 rounded-2xl overflow-hidden border-2 border-dashed border-border/40 bg-elevated/20 transition-colors hover:bg-elevated/40 hover:border-primary/40 group">
                    ...
                    <div className="grid lg:grid-cols-2 gap-6">
                       <div className="glass-2 p-6 border-border/40">
               ...
               {tab === "SIZES" && (
                 <div className="space-y-6 animate-fade-in relative z-10">
                    <div className="flex items-center justify-between">
                       ...
                       <button onClick={addSize} className="glass-btn glass-btn-primary !rounded-full px-5 text-xs font-black">
               ...
               {tab === "MODIFIERS" && (
                 <div className="space-y-8 animate-fade-in relative z-10">
                    <div className="flex items-center justify-between">
                       ...
                       <button onClick={addModGroup} className="glass-btn glass-btn-primary !rounded-full px-5 text-xs font-black">
                    ...
                    <div className="space-y-6">
                       ...
                       {(item.modifierGroups || []).map(group => (
                          <div key={group.id} className="glass-2 rounded-[2.5rem] overflow-hidden border-border/30 shadow-sm relative group">
                             ...
                             <div className="p-6 space-y-3 bg-card/10">
               ...
               {tab === "RECIPE" && (
                 <div className="space-y-8 animate-fade-in relative z-10">
                    ...
                    <div className="glass-2 p-8 border-border/40">
                        ...
                        <div className="flex flex-col sm:flex-row gap-4 mb-8 bg-elevated/40 p-5 rounded-3xl border border-border/30">
                           <div className="flex-1">
                              ...
                              <select value={recipeIngredientId} onChange={e => setRecipeIngredientId(e.target.value)} className={`${inputCls} h-12 text-sm font-bold`}>
                                 <option value="">{lang === 'ar' ? 'اختر من الأصناف المخزنية...' : 'Choose data asset...'}</option>
                        ...
                        <div className="space-y-3">
                           {activeRecipeIngredients().map((ri: any) => {
                             ...
                             return (
                               <div key={ingredientId} className="flex items-center p-4 rounded-2xl bg-elevated border border-border/30 group">
                    ...
                    <div className="glass-2 p-8 border-border/40">
                      ...
                      <div className="grid gap-4">
                        ...
                        {branches.map(branch => {
                          ...
                          return (
                            <div key={branch.id} className="flex items-center justify-between p-5 rounded-2xl bg-elevated/40 border border-border/30 hover:border-primary/20 transition-colors">
                              <div className="flex items-center gap-4">
                    ...
                    <div className="glass-2 p-8 border-border/40">
                      ...
                      <div className="grid sm:grid-cols-2 gap-5">
                         {platformOptions.map(p => {
                           ...
                           return (
                             <div key={p.id} className="p-6 rounded-3xl bg-elevated/40 border border-border/30 hover:shadow-md hover:border-indigo-500/20 transition-all flex flex-col gap-5">
                               <div className="flex items-center gap-3">
               ...
               {tab === "HISTORY" && (
                 <div className="space-y-6 animate-fade-in relative z-10">
                    <div className="glass-2 p-8 border-border/40 min-h-[400px]">
                       ...
                       <div className="space-y-4">
                          {(!item.versionHistory || item.versionHistory.length === 0) ? (
                             ...
                             [...(item.versionHistory)].reverse().map((entry, i) => (
                               <div key={i} className="p-5 rounded-2xl bg-elevated/40 border border-border/30 flex items-start gap-4 hover:bg-elevated transition-colors">
        ...
        {/* Bottom Actions Bar */}
        <div className="shrink-0 p-5 px-8 border-t border-border/30 bg-card/80 backdrop-blur-xl relative z-20">
           <div className="flex items-center justify-between max-w-4xl mx-auto w-full">
              <div className="flex items-center gap-3">
                 <button onClick={onClose} className="h-12 px-8 rounded-xl font-black text-xs uppercase tracking-widest text-muted bg-elevated/60 border border-border/40 hover:text-main hover:bg-elevated hover:shadow-sm transition-all">
                    {lang === 'ar' ? 'رجوع' : 'Cancel'}
                 </button>
                 {onDelete && mode === "EDIT" && (
                    <button onClick={onDelete} className="h-12 w-12 flex items-center justify-center rounded-xl bg-rose-500/10 text-rose-500 border border-rose-500/20 hover:bg-rose-500 hover:text-white shadow-sm transition-all cursor-pointer">
                       <Trash2 size={18} />
                    </button>
                 )}
              </div>
              <div className="flex items-center gap-3">
                 <button 
                   onClick={() => { if (item.name?.trim()) onSave(item, activeCategoryId, false); }}
                   disabled={!item.name?.trim()}
                   className="h-12 px-6 rounded-xl font-black text-xs uppercase tracking-widest bg-elevated/80 border border-border/50 text-main hover:bg-elevated transition-all shadow-sm active:scale-95 disabled:opacity-50"
                 >
                 ...
                 <button 
                   ...
                   disabled={!item.name?.trim()}
                   className="h-12 px-10 rounded-xl font-black text-xs uppercase tracking-[0.2em] glass-btn-primary transition-all active:scale-95 disabled:opacity-50 flex items-center gap-2"
                 >
                    <Sparkles size={16} />
                    {lang === 'ar' ? 'حفظ ونشر' : 'Commit'}
                 </button>
              </div>
           </div>
        </div>
